...
const splatterSprite = Sprite.fromUrl(
      "https://s3-us-west-2.amazonaws.com/s.cdpn.io/163870/blood-splatter-0.png");

let isSlicing = true;

const zombieHeadImages = [
  "https://s3-us-west-2.amazonaws.com/s.cdpn.io/163870/s-zombie-0-3.png",
  "https://s3-us-west-2.amazonaws.com/s.cdpn.io/163870/s-zombie-0-4.png",
  "https://s3-us-west-2.amazonaws.com/s.cdpn.io/163870/s-zombie-0-5.png",
  "https://s3-us-west-2.amazonaws.com/s.cdpn.io/163870/s-zombie-0-6.png",
  "https://s3-us-west-2.amazonaws.com/s.cdpn.io/163870/s-zombie-0-7.png",
  "https://s3-us-west-2.amazonaws.com/s.cdpn.io/163870/s-zombie-0-8.png",
];

const humanHeadImages = [
  "https://s3-us-west-2.amazonaws.com/s.cdpn.io/163870/baby.png"
];
...
class GameBackground extends GameObject {
  constructor() {
    super();    
    this.bgSprite = Sprite.fromUrl("https://s3-us-west-2.amazonaws.com/s.cdpn.io/163870/bg-wall.jpg");
...
class Game extends GameObject {
  ...
  update() {    
    ...
    setGravityMultiplier(0.025 +  (game.score/45000));
  }
  addBloodStain(x, y) {        
...
class SliceableObject extends GameObject {
  constructor(sprite, shape) {
    ...
    this.sprite.position.x = canvas.width/2-this.sprite.width/2;
  ...
  update() {
    ...
    if (this.sprite.position.y> canvas.height + this.sprite.height ) {
      if (!this.sliced) game.removeHealth();
      this.destroy();
    }
  }  
  slice(scene, line) {
...
class ZombieHead extends SliceableObject {
  constructor() {
    super(Sprite.fromUrl(zombieHeadImages[Math.floor(zombieHeadImages.length * Math.random())]), createZombieHeadPolygon());
    this.score = 100;
  }
  draw() {
    if (!this.isEnabled || !this.isVisible) return;
...
class HumanHead extends SliceableObject {
  constructor() {
    super(Sprite.fromUrl(humanHeadImages[Math.floor(humanHeadImages.length * Math.random())]),createHumanHeadPolygon());
    this.score = -500;
  }
  draw() {
    if (!this.isEnabled || !this.isVisible) return;
  ...
  updateSlice(game) {
    ...
    for(let obj of game.children) {
      ...
      if (slicePoints.length > 1) {
        ...
        game.addBloodStain(splatterPos.x, splatterPos.y);
      }
    }         
  }
...
function createHumanHeadPolygon() {
  return new Polygon([new Point(21,72),new Point(62,32),new Point(100,4),new Point(146,24),new Point(152,54),new Point(182,96),new Point(189,116),new Point(177,148),new Point(188,206),new Point(189,284),new Point(148,362),new Point(106,386),new Point(44,377),new Point(18,340),new Point(16,308),new Point(2,297),new Point(4,174),new Point(14,133),new Point(9,86),new Point(21,72)]);
}
